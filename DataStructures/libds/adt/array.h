...
namespace ds::adt {
	...
	{
		return index >= base_ && (index) < static_cast<long long>(base_ + this->size());
	}
	...
	{
		return index1 >= dimension1_.getBase() && index1 < static_cast<long long>(dimension1_.getBase() + dimension1_.getSize())
			&& index2 >= dimension2_.getBase() && index2 < static_cast<long long>(dimension2_.getBase() + dimension2_.getSize());
	}
